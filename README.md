...
```
You wil then be told how manhy e's appear in the above text file.
...
So, overall this program calculates how many e's are in the testing.txt file by importing the sys module which allows for command line aerguments (sys.argv). Once set up correctly as above - simply type:
```sh
python .\letterfrequency.py testing.txt e
```
